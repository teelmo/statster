$.extend(view, {
  ...
  populateMonthPicker: () => {
    for (i = 0; i < 12; i++) {
      $('.month_selector').append(`<option value="${i + 1}" ${parseInt(`<?=(isset($month) ? $month : 'false')?>`, 10) === i + 1 ? 'selected="selected"' : ''}>${moment.monthsShort(i)}</option>`);
    }
  },
  populateDayPicker: () => {
    function ordinal(n) {
    ...
    for (i = 0; i < 31; i++) {
      $('.day_selector').append(`<option class="day_option" value="${i + 1}" ${parseInt(`<?=(isset($day) ? $day : 'false')?>`, 10) === i + 1 ? 'selected="selected"' : ''}>${ordinal(i + 1)}</option>`);
    }
  },
  populateWeekdayPicker: () => {
    for (i = 0; i < 7; i++) {
      $('.weekday_selector').append(`<option value="${i + 1}" ${parseInt(`<?=(isset($weekday) ? $weekday : 'false')?>`, 10) === i ? 'selected="selected"' : ''}>${moment.weekdays(i + 1)}</option>`);
    }
  },
  ...
  initDateFilterHelperEvents: () => {
    function checkIfFilterActive() {
      return !!($('.month_selector option:selected').val() === '' && $('.day_selector option:selected').val() === '' && $('.weekday_selector option:selected').val() === '' && $('.date_range_picker').html() === 'All time');
    }
    $('.date_filter_clear').click(event => {
    ...
    $('.month_selector').change(function (_event) {
      if ($(this).val() !== '') {
    ...
    $('.day_selector').change(function (_event) {
      if ($(this).val() !== '') {
        $('.date_filter_clear').show();
      } else if (checkIfFilterActive()) {
        $('.date_filter_clear').hide();
    ...
    $('.weekday_selector').change(function (_event) {
      if ($(this).val() !== '') {
        $('.date_filter_clear').show();
      } else if (checkIfFilterActive()) {
        $('.date_filter_clear').hide();
    ...
    $('.date_filter_submit').click(_event => {
      var filter = [];
      ...
      if ($('.date_range_picker').html() !== 'All time') {
        const dates = $('.date_range_picker').html().split(' to ');
