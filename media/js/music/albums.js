Object.assign(view, {
  ...
  getTopAlbumYearly: () => {
    ...
    for (year = parseInt(`<?=CUR_YEAR?>`, 10); year >= 2003; year--) {
      document
        ...
        .insertAdjacentHTML(
          'beforeend',
          `<div class="container"><h2 class="number">${year}</h3><div class="lds-facebook inline" id="sideTopAlbum${year}Loader"><div></div><div></div><div></div></div><table id="sideTopAlbum${year}" class="side_table"></table><div class="more"><a href="/album/${year}" title="Browse more">More <span class="number">${year}</span></</a></div></div><div class="container"><hr /></div>`
        );
      vars = {
      ...
      document
        ...
        .insertAdjacentHTML(
          ...
          '<div class="container"><h2 class="number">' +
            month_str[month] +
            '</h3><div class="lds-facebook inline" id="sideTopAlbum' +
            month +
  ...
  getTopAlbumDaily: (year, month) => {
    ...
    for (day = 1; day <= new Date(year, month, 0).getDate(); day++) {
      ...
      document
        ...
        .insertAdjacentHTML(
          'beforeend',
          '<div class="container"><div><div class="lds-facebook inline" id="sideTopAlbum' +
            day +
  ...
  getUsers: (date, _vars) => {
    document.querySelector('#sideTable').insertAdjacentHTML('beforeend', '<div class="container"><div class="lds-facebook inline" id="topListenerLoader"><div></div><div></div><div></div></div><table id="topListener" class="side_table"><!-- Content is loaded with AJAX --></table></div>');
    ajax({
      data: {
        limit: 14,
        lower_limit: date,
