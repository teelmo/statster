$.extend(view, {
  ...
  getTopAlbumDaily: function (year, month) {
    ...
    for (var day = 1; day <= new Date(year, month, 0).getDate(); day++) {
      ...
      $('<div class="container"><div><div class="lds-facebook" id="sideTopAlbum' + day + 'Loader"><div></div><div></div><div></div></div><span id="sideTopAlbum' + day + '" class="number"></span> listenings <div class="metainfo">' + weekday[new Date(year, month, day).getDay()] + ' – <span class="number">' + app.getGetOrdinal(day) + '</span></div></div><div class="more"><a href="/album/' + year + '/' + pad_month + '/' + pad_day + '" title="Browse more">More</a></div></div>').appendTo($('#sideTable'));
      var vars = {
