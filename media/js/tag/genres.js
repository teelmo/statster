Object.assign(view, {
  ...
  getTopGenresYearly: () => {
    for (year = parseInt(`<?=CUR_YEAR?>`, 10); year >= 2003; year--) {
      document.querySelector('#years').insertAdjacentHTML('beforeend', `<div class="container"><h2 class="number">${year}</h3><div class="lds-facebook" id="topGenre${year}Loader"><div></div><div></div><div></div></div><table id="topGenre${year}" class="side_table"></table></div><div class="container"><hr /></div>`);
      const vars = {
