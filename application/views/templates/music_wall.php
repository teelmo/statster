...
if (!empty($json_data)) {
  if (is_array($json_data)) {
    ...
    foreach ($json_data as $data) {
      if ($type == 'album') {
        echo '<li>' . anchor(array('music', url_title($data['artist_name']), url_title($data['album_name'])), '<div class="cover album_img img150" style="background-image:url(' . getAlbumImg(array('album_id' => $data['album_id'], 'size' => 174)) . ')"></div><div class="meta"><div class="title main">' . anchor(array('music', url_title($data['artist_name']), url_title($data['album_name'])), substrwords($data['album_name'], 35), array('title' => 'Browse to album\'s page')) . '</div><div class="title">' . anchor(array('music', url_title($data['artist_name'])), $data['artist_name'], array('title' => 'Browse to artist\'s page')) . '</div><div class="title count">' . $data['count'] . ' '. $word . '</div></div>', array('title' => 'Browse to album\'s page')) . '</li>';
      }
      elseif ($type == 'artist') {
