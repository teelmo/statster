  ...
  if (is_array($json_data)) {
    ...
    if ($first = array_shift($json_data)) {
      ...
      elseif ($type == 'artist') {
        ?>
        <li>
          <?=anchor(array('music', url_title($first['artist_name'])), '<span></span>', array('title' => 'Browse to artist\'s page'))?>
          <div class="cover artist_img img300" style="background-image:url(<?=getArtistImg(array('artist_id' => $first['artist_id'], 'size' => 300))?>)"><div class="meta"><div class="title main"><?=anchor(array('music', url_title($first['artist_name'])), $first['artist_name'], array('title' => 'Browse to artist\'s page'))?></div><div class="title count"><?=$first['count']?> <?=$word?></div></div></div>
        </li>
        <?php
      }
    }
    foreach ($json_data as $data) {
      ...
      elseif ($type == 'artist') {
        ?>
        <li>
          <?=anchor(array('music', url_title($data['artist_name'])), '<span></span>', array('title' => 'Browse to artist\'s page'))?>
          <div class="cover artist_img img150" style="background-image:url(<?=getArtistImg(array('artist_id' => $data['artist_id'], 'size' => 174))?>)"><div class="meta"><div class="title main"><?=anchor(array('music', url_title($data['artist_name'])), substrwords($data['artist_name'], 35), array('title' => 'Browse to artist\'s page'))?></div><div class="title count"><?=$data['count']?> <?=$word?></div></div></div>
          </li>
        <?php
      }
    }
