  ...
  if (is_array($json_data)) {
    ...
    foreach ($json_data as $idx => $row) {
      ...
      <li class="album">
        ...
        <?=anchor(array('music', url_title($row['artist_name']), url_title($row['album_name'])), '<span></span>', array('title' => 'Browse to album\'s page'))?>
        <div class="cover album_img img150" style="background-image:url(<?=getAlbumImg(array('album_id' => $row['album_id'], 'size' => 174))?>)"><div class="meta"><div class="title main"><?=anchor(array('music', url_title($row['artist_name']), url_title($row['album_name'])), substrwords($row['album_name'], 35), array('title' => 'Browse to album\'s page'))?></div><?=((isset($hide) && $hide['artist'] == 'true') ? '' : ('<div class="title">' . implode('<span class="artist_separator">, </span>', array_map(function($artist) { return anchor(array('music', url_title($artist['artist_name'])), $artist['artist_name'], array('title' => 'Browse to artist\'s page'));}, isset($album_artists[$row['album_id']]) ? $album_artists[$row['album_id']] : array())) . '</div>'))?></div></div>
