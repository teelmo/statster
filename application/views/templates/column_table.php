      ...
      <tr id="column_table_<?=$idx?>">
        ...
        <td class="name">
          ...
          if (isset($type) && $type === 'user') {
            echo anchor(array('user', url_title($row['username'])), html_escape($row['username']), array('title' => 'Browse to user\'s page'));
          }
            ...
            if ($row['type'] === 'star') {
              ...
              echo (empty($row['format_type_name'])) ? anchor(array('format', url_title(str_replace(' ', '-', $row['format_name']))), '<img src="/media/img/format_img/format_icons/' . $listeningsFormatImg . '.png" alt="" title="' . $row['format_type_name'] . '" class="middle icon listening_format_type"/>' . $row['format_name'], array('title' => 'Browse to format\'s page')) : anchor(array('format', url_title(str_replace(' ', '-', $row['format_name'])), url_title($row['format_type_name'])), '<img src="/media/img/format_img/format_icons/' . $listeningsFormatTypeImg . '.png" alt="" title="' . $row['format_type_name'] . '" class="middle icon listening_format_type"/>' . $row['format_type_name'], array('title' => 'Browse to format type\'s page')) . (empty($hide['format_icon']) ? anchor(array('format', url_title(str_replace(' ', '-', $row['format_name']))), '<img src="/media/img/format_img/format_icons/' . $listeningsFormatImg . '.png" alt="" title="' . $row['format_type_name'] . '" class="middle icon float_right listening_format_type"/>', array('title' => 'Browse to format\'s page')) : '');
            }
            else {
